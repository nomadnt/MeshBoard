...
class Date {
    ...
    public static function fuzzy_span2($timestamp, $local_timestamp = NULL){
        $time = NULL;
        $span = Date::span($timestamp, $local_timestamp);
        if($span['years'] > 1) return sprintf('%d years', $span['years']);
        if($span['months'] > 1) return sprintf('%d months', $span['months']);
        if($span['weeks'] > 1) return sprintf('%d weeks', $span['weeks']);
        if($span['days'] > 1) $time.= sprintf('%d days ', $span['days']);
        $time.= sprintf('%02d:%02d:%02d ', $span['hours'], $span['minutes'], $span['seconds']);
        return $time;
    }
}
